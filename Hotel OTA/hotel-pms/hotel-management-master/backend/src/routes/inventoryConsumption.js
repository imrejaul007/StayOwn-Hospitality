...
router.post('/', authenticate, validate(mutationBaselineSchema), async (req, res) => {
  try {
    ...
    logger.error('Error tracking consumption', { error: error.message });
    res.status(500).json({
      success: false,
...
router.post('/housekeeping/predict', authenticate, validate(mutationBaselineSchema), async (req, res) => {
  try {
    ...
    logger.error('Error predicting consumption', { error: error.message });
    res.status(500).json({
      success: false,
...
router.get('/housekeeping/analytics', authenticate, async (req, res) => {
  try {
    ...
    logger.error('Error getting housekeeping analytics', { error: error.message });
    res.status(500).json({
      success: false,
...
router.get('/housekeeping/trends', authenticate, async (req, res) => {
  try {
    ...
    logger.error('Error getting consumption trends', { error: error.message });
    res.status(500).json({
      success: false,
...
router.get('/guest/recommendations', authenticate, async (req, res) => {
  try {
    ...
    logger.error('Error getting guest recommendations', { error: error.message });
    res.status(500).json({
      success: false,
...
router.get('/guest/analytics', authenticate, async (req, res) => {
  try {
    ...
    logger.error('Error getting guest analytics', { error: error.message });
    res.status(500).json({
      success: false,
...
router.post('/auto/housekeeping', authenticate, validate(mutationBaselineSchema), async (req, res) => {
  try {
    ...
    logger.error('Error auto-consuming inventory', { error: error.message });
    res.status(500).json({
      success: false,
...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    logger.error('Error getting consumption records', { error: error.message });
    res.status(500).json({
      success: false,
...
router.get('/stats', authenticate, async (req, res) => {
  try {
    ...
    logger.error('Error getting consumption stats', { error: error.message });
    res.status(500).json({
      success: false,
...
router.get('/:id', authenticate, async (req, res) => {
  try {
    ...
    logger.error('Error getting consumption record', { error: error.message });
    res.status(500).json({
      success: false,
...
router.put('/:id/billing', authenticate, validate(mutationBaselineSchema), async (req, res) => {
  try {
    ...
    logger.error('Error updating billing status', { error: error.message });
    res.status(500).json({
      success: false,
