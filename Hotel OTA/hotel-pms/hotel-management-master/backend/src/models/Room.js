...
roomSchema.methods.getRoomType = async function() {
  try {
    ...
    if (this.roomTypeId) {
      ...
      return await RoomType.findById(this.roomTypeId).lean();
    }
    ...
    if (roomTypeFilter) {
      ...
      if (availabilityRecords.length > 0) {
        ...
        if (minAvailable > 0) {
          ...
          return await this.find(query).populate('roomTypeDetails').sort({ roomNumber: 1 }).lean().limit(1000);
        }
      }
    }
    ...
    if (doc._wasStatusModified) {
      ...
      if (notification) {
        ...
        if (doc.status === 'out_of_order') {
          try {
            const RoomType = mongoose.model('RoomType');
            const roomType = await RoomType.findById(doc.roomTypeId).select('basePrice name').lean();
