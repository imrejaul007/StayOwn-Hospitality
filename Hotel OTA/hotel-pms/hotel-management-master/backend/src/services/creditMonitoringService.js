...
class CreditMonitoringService {
  ...
  async monitorCreditStatus(hotelId) {
    try {
      ...
      logger.error('Credit monitoring error:', error);
      throw error;
    }
  }
  ...
  async createStaffAlerts(hotelId, results) {
    try {
      ...
      logger.error('Error creating staff alerts:', error);
    }
  }
  ...
  async processBookingCredit(companyId, amount, bookingId, userId, auditContext = {}) {
    try {
      logger.debug('Starting processBookingCredit with params:', { companyId, amount, bookingId, userId });
      ...
      logger.debug('Credit validation result:', validation);
      ...
      logger.debug('Company found:', { id: company._id, name: company.name, availableCredit: company.availableCredit });

      company.availableCredit -= amount;
      await company.save();
      logger.debug('Company credit updated, new balance:', company.availableCredit);
      ...
      logger.debug('Creating credit transaction with data:', transactionData);

      const creditTransaction = await CorporateCredit.create(transactionData);
      logger.debug('Credit transaction created successfully:', creditTransaction._id);
      ...
      logger.error('Credit processing error:', error);
  ...
  async requestCreditLimitIncrease(companyId, requestedLimit, requestedBy, justification, auditContext = {}) {
    try {
      ...
      logger.error('Credit limit request error:', error);
      return {
        success: false,
  ...
  async processCreditLimitRequest(requestId, action, approvedBy, comments) {
    try {
      ...
      logger.error('Credit limit approval error:', error);
      return {
        success: false,
  ...
  async processCreditAdjustment(companyId, adjustmentAmount, reason, adjustedBy, auditContext = {}) {
    try {
      ...
      logger.error('Credit adjustment error:', error);
      return {
        success: false,
  ...
  async sendLowCreditNotifications(hotelId, notificationConfig = {}) {
    try {
        ...
        if (alertLevel) {
          ...
          if (emailNotifications && recipients.length > 0) {
            try {
              ...
              logger.debug('Email notification prepared:', emailData);
          ...
          if (smsNotifications) {
            // SMS integration
            logger.debug('SMS notification would be sent for:', company.name);
          }

          if (slackNotifications) {
            // Slack integration
            logger.debug('Slack notification would be sent for:', company.name);
          }

          if (webhookNotifications) {
            // Webhook integration
            logger.debug('Webhook notification would be sent for:', company.name);
          }
        }
      }
      ...
      logger.error('Low credit notification error:', error);
      return {
        success: false,
        error: error.message
      };
  ...
  async scheduleAutomatedMonitoring(hotelId, scheduleConfig = {}) {
    try {
      ...
      logger.debug('Monitoring schedule configured:', monitoringSchedule);
  ...
  async getCreditAlertHistory(hotelId, filters = {}) {
    try {
      ...
      logger.error('Credit alert history error:', error);
      return {
        success: false,
        error: error.message
      };
  ...
  async getCreditMonitoringSummary(hotelId) {
    try {
      ...
      logger.error('Error getting monitoring summary:', error);
      throw error;
    }
  }
  ...
  async verifyTransactionIntegrity(transactionId, auditContext = {}) {
    try {
      ...
      logger.error('Transaction integrity verification error:', error);
      await creditSecurityService.logCreditOperation({
  ...
  async batchVerifyTransactionIntegrity(transactionIds, auditContext = {}) {
    try {
      ...
      logger.error('Batch transaction integrity verification error:', error);
      return {
        total: transactionIds.length,
        valid: [],
        invalid: [],
  ...
  async runDailyIntegrityAudit(hotelId, auditContext = {}) {
    try {
      ...
      logger.error('Daily integrity audit error:', error);
      return {
        success: false,
        error: error.message,
        total: 0,
  ...
  async processSecureTransaction(transactionData, sensitiveFields = [], auditContext = {}) {
    try {
      ...
      logger.error('Secure transaction processing error:', error);
      return {
        success: false,
        error: error.message
      };
