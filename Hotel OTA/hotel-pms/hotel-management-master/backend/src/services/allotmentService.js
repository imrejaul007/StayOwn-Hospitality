...
class AllotmentService {
  ...
  async createAllotment(allotmentData, userId) {
    try {
      ...
      try {
        ...
        logger.warn('Failed to log action:', logError.message);
      }
  ...
  async applyDynamicAllocation(allotment, dailyAllotment, rule, date) {
    try {
      ...
      logger.warn(`Dynamic allocation failed, falling back to percentage: ${error.message}`);
      const fallbackRule = {
        type: 'percentage',
        allocation: {
          percentage: new Map([
  ...
  async generateAnalytics(allotmentId, period) {
    try {
      ...
      logger.debug('Generating analytics for allotment:', allotmentId, 'period:', period);
      ...
      for (const channel of allotment.channels) {
        if (channel.isActive) {
          ...
          if (periodAllotments.length === 0) {
            logger.debug(`No data found in requested period, using all available data`);
            periodAllotments = allotment.dailyAllotments;
          }
          
          logger.debug(`Channel ${channel.channelId}: Found ${periodAllotments.length} days in period`);
  ...
  async calculateLeadTimeDistribution(periodAllotments, hotelId) {
    try {
      ...
      logger.error('Error calculating lead time distribution:', error);
  ...
  async getDashboard(hotelId, params = {}) {
    try {
      logger.debug('🔍 [AllotmentService] getDashboard called with hotelId:', hotelId);
      ...
      logger.debug('✅ [AllotmentService] Returning dashboard data:', dashboardData);

      return dashboardData;
    } catch (error) {
      logger.error('❌ [AllotmentService] Error fetching dashboard data:', error);
  ...
  async logAction(allotmentId, userId, action, details) {
    try {
      ...
      logger.error('Failed to log allotment action:', error.message);
    }
  }
}
