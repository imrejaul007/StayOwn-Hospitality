...
class DynamicPricingService {
  ...
  async calculateOptimalRates(hotelId, startDate, endDate, options = {}) {
    try {
      ...
      logger.debug(`🧮 Calculating optimal rates for hotel ${hotelId}...`);
      ...
      for (const date of dateRange) {
        for (const roomType of roomTypes) {
          try {
            ...
            logger.error(`Error calculating rate for ${roomType.name} on ${date}:`, roomError);
          }
        }
      }
      ...
      logger.error('Error calculating optimal rates:', error);
      return {
        success: false,
        error: error.message
      };
  ...
  async calculateRoomTypeRate(hotelId, roomType, date, strategies, options) {
    try {
      ...
      logger.error('Error calculating room type rate:', error);
      throw error;
    }
  }
  ...
  async getDemandAdjustment(hotelId, roomTypeId, date) {
    try {
      ...
      logger.error('Error getting demand adjustment:', error);
      return null;
    }
  }
  ...
  async getCompetitorAdjustment(hotelId, roomType, date) {
    try {
      ...
      logger.error('Error getting competitor adjustment:', error);
      return null;
    }
  }
  ...
  async applyPricingChanges(hotelId, pricingResults, options = {}) {
    try {
      ...
      logger.debug(`💰 Applying pricing changes for hotel ${hotelId}...`);
      ...
      for (const pricing of pricingResults) {
        try {
          ...
          logger.error(`Error applying price change for ${pricing.roomTypeName}:`, changeError);
          skippedChanges.push({
            ...pricing,
      ...
      logger.error('Error applying pricing changes:', error);
      return {
        success: false,
        error: error.message
      };
  ...
  async updateRoomAvailabilityRate(hotelId, roomTypeId, date, newRate, pricingContext) {
    try {
      ...
      logger.error('Error updating room availability rate:', error);
      return false;
    }
  }
  ...
  async syncPricingToChannels(hotelId, appliedChanges) {
    try {
      logger.debug(`🔄 Syncing pricing changes to channels...`);
      ...
      logger.error('Error syncing pricing to channels:', error);
      return {
        success: false,
        error: error.message
      };
  ...
  async runAutomatedPricing(hotelId, options = {}) {
    if (this.isRunning) {
      logger.debug('Automated pricing already running');
      return { success: false, message: 'Already running' };
    }

    try {
      this.isRunning = true;
      logger.debug(`🤖 Starting automated pricing for hotel ${hotelId}...`);
      ...
      logger.error('Error in automated pricing:', error);
      return {
        success: false,
        error: error.message
      };
