...
export const toggleCorporateCompanyStatus = catchAsync(async (req, res, next) => {
  ...
  // If activating, no checks needed - use atomic update
  if (!existing.isActive) {
    const company = await CorporateCompany.findOneAndUpdate(
      { _id: req.params.id, hotelId: req.user.hotelId },
      { $set: { isActive: true, 'metadata.lastModifiedBy': req.user.id } },
      { new: true }
    );
  ...
  // Safe to deactivate - use atomic update
  const company = await CorporateCompany.findOneAndUpdate(
    { _id: req.params.id, hotelId: req.user.hotelId },
    { $set: { isActive: false, 'metadata.lastModifiedBy': req.user.id } },
    { new: true }
  );
