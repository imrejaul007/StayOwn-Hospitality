...
class BillMessageController {
  ...
  deleteMessage = catchAsync(async (req, res, next) => {
    ...
    if (isUsed || message.isSystemMessage) {
      ...
      const deactivated = await BillMessage.findOneAndUpdate(
      ...
      logger.info('Bill message deactivated', {
        messageId: deactivated._id,
        messageName: deactivated.name,
        userId: req.user._id
      });
  ...
  addTranslation = catchAsync(async (req, res, next) => {
    ...
    try {
      // Atomically remove existing translation for this language and add new one
      // First, pull any existing translation for this language
      ...
      const updated = await BillMessage.findOneAndUpdate(
        { _id: messageId, hotelId: req.user.hotelId },
        {
          $push: {
            'localization.translations': { language, content, htmlContent, title }
          },
