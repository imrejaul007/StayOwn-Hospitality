...
class EnhancedDemandForecastingService {
  ...
  async generateAdvancedDemandForecast(filters = {}) {
    try {
      ...
      logger.error('Error in generateAdvancedDemandForecast:', error);
      return {
        success: false,
  ...
  async getMarketTrendIndicators(hotelId) {
    try {
      ...
      logger.error('Error getting market trend indicators:', error);
      return {};
    }
  }
  ...
  async getBookingPaceAnalysis(hotelId) {
    try {
      ...
      logger.error('Error getting booking pace analysis:', error);
  ...
  async generateMLForecast(data) {
    try {
      ...
      logger.error('Error generating ML forecast:', error);
      return [];
    }
  }
