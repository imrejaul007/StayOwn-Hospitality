...
router.get('/occupancy', authorize('admin', 'manager', 'staff'), catchAsync(async (req, res, next) => {
  ...
  logger.debug('Room filter constructed', { hotelId: roomFilter.hotelId });
  ...
  logger.debug('Rooms with status found', { count: roomsWithStatus.length });
  ...
  logger.debug('Room count for hotel', { roomCount });
  ...
  logger.debug('Final occupancy metrics', { overallMetrics });
...
router.get('/revenue', authorize('admin', 'manager', 'staff'), catchAsync(async (req, res, next) => {
  ...
  // NOTE: Uses compound index { hotelId: 1, status: 1, createdAt: -1 } on Booking
...
router.get('/alerts', authorize('admin', 'manager', 'staff'), catchAsync(async (req, res, next) => {
  ...
  // NOTE: Uses compound index { hotelId: 1, paymentStatus: 1, checkOut: -1 } on Booking
...
router.get('/system-health', authorize('admin', 'manager'), catchAsync(async (req, res, next) => {
  ...
  const generateTrendData = async (metricType, baseValue, variation = 0.2, points = 24) => {
    try {
      ...
      logger.error(`Error generating trend data for ${metricType}`, { error: error.message });
...
router.post('/bypass-checkout', authenticate, authorizePolicy('adminDashboard', 'adminFrontdeskAccess'), validate(bypassCheckoutSchema), catchAsync(async (req, res) => {
  ...
  logger.info('Admin bypass checkout initiated', { bookingId, paymentMethod, adminId });
  ...
  logger.info('Admin bypass checkout completed', {
    bookingId: booking._id,
...
async function generateSystemAlerts(hotelId, currentTime) {
  ...
  try {
    ...
    logger.error('Error generating system alerts', { error: error.message });
