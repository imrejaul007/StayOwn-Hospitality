...
class AddOnController {
  ...
  async bulkCreateServices(req, res) {
    try {
      ...
      // Batch insert: prepare all documents up front, then use insertMany
      const { v4: uuidv4 } = await import('uuid');
      const documents = services.map(serviceData => ({
        ...serviceData,
      ...
      const results = [];
      try {
        const created = await AddOnService.insertMany(documents, { ordered: false });
        ...
        if (bulkError.insertedDocs) {
          bulkError.insertedDocs.forEach(service => {
            results.push({ success: true, data: service });
          });
        }
        // Record individual write errors
        if (bulkError.writeErrors) {
          bulkError.writeErrors.forEach(err => {
            results.push({
              success: false,
              error: err.errmsg || err.message,
              data: documents[err.index]
            });
          });
        }
      }
