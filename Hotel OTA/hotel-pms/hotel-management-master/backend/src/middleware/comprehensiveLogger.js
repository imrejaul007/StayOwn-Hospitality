...
export const comprehensiveAPILogger = (options = {}) => {
  ...
  return (req, res, next) => {
    ...
    const requestData = {
      ...
      path: req.path,
      query: JSON.parse(safeStringify(req.query)),
      params: req.params,
    ...
    // Clone at capture time to avoid mutations from downstream middleware (e.g., tenantIsolation)
    ...
    function captureResponse(body, method, encoding) {
      ...
      if (logPayloads && body) {
        ...
        if (bodySize <= maxPayloadSize) {
          responseBody = method === 'json' ? JSON.parse(safeStringify(body)) : bodyStr;
        }
      }
      ...
      const responseData = {
        ...
        duration,
        responseSize: responseBody ? Buffer.byteLength(safeStringify(responseBody)) : 0,
        method: method,
        encoding
      };
...
/**
 * Safe JSON.stringify that handles circular references and MongoDB ObjectIds
 */
function safeStringify(obj) {
  const seen = new WeakSet();
  return JSON.stringify(obj, (key, value) => {
    // Convert MongoDB ObjectIds and Buffers to strings
