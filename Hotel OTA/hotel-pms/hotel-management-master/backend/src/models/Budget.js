...
budgetSchema.methods.calculateTotals = async function() {
  try {
    ...
    const accountIds = this.budgetLines.map(l => l.accountId);
    const accounts = await ChartOfAccounts.find({ _id: { $in: accountIds } }).lean();
...
budgetSchema.statics.getBudgetVsActual = async function(hotelId, fiscalYear, period) {
  try {
    ...
    }).populate('budgetLines.accountId').lean();
