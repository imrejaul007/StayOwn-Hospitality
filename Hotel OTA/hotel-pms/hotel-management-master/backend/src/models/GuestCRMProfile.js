...
guestCRMProfileSchema.pre('save', function(next) {
  ...
  const sensitiveFields = ['passportNumber', 'aadhaarNumber', 'panCard', 'driverLicense', 'nationalId'];
  for (const field of sensitiveFields) {
    if (this.isModified(field) && this[field] && !pii.isEncrypted(this[field])) {
      this[field] = pii.encrypt(this[field]);
    }
  }
  next();
});
...
guestCRMProfileSchema.methods.getMaskedPII = function(fieldName) {
  const pii = getPIIEncryption();
  return this[fieldName] ? pii.mask(this[fieldName]) : null;
};
