    ...
    for (const booking of multiBooking.bookings) {
      if (booking.bookingId && booking.status === 'confirmed') {
        ...
        if (individualBooking) {
          ...
          if (validation.valid) {
            try {
              await atomicStatusTransition(Booking, booking.bookingId, individualBooking.status, 'cancelled', {
                cancellationReason: reason || 'Group booking cancelled'
              });
            } catch (transitionError) {
              logger.warn(`Could not cancel booking ${booking.bookingId}: ${transitionError.message}`);
            }
          }
        }
  ...
  try {
    await session.withTransaction(async () => {
      ...
      for (const booking of multiBooking.bookings) {
        if (booking.bookingId && booking.status === 'confirmed') {
          try {
            // Cancel main booking using atomic state machine
            const individualBooking = await Booking.findById(booking.bookingId).session(session);
            if (individualBooking) {
              const validation = validateTransition(individualBooking.status, 'cancelled');
              if (!validation.valid) {
