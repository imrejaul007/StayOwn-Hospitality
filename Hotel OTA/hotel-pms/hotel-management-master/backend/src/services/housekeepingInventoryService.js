...
class HousekeepingInventoryService {
  ...
  async predictConsumption(hotelId, roomId, taskTypes = []) {
    try {
      // Get room details
      const room = await Room.findById(roomId).populate('roomType').lean();
      if (!room) {
  ...
  async autoConsumeOnTaskCompletion(housekeepingTaskId) {
    try {
      ...
      // Batch: fetch all recommended items in a single query
      const highConfRecs = predictions.recommendations.filter(
        r => r.confidence >= 70 && r.recommendedQuantity > 0
      );
      const recItemIds = highConfRecs.map(r => r.inventoryItem._id);
      const recItems = recItemIds.length > 0
        ? await InventoryItem.find({ _id: { $in: recItemIds } }).limit(1000).lean()
        : [];
      const recItemMap = new Map(recItems.map(it => [it._id.toString(), it]));

      for (const recommendation of highConfRecs) {
        const item = recItemMap.get(recommendation.inventoryItem._id.toString());
        if (item && item.currentStock >= recommendation.recommendedQuantity) {
          autoConsumptions.push({
  ...
  async createLowStockAlert(inventoryItem, session) {
    try {
      ...
      logger.debug(`Low stock alert for ${inventoryItem.name}: ${inventoryItem.currentStock} remaining`);
  ...
  async updateTaskEfficiencyMetrics(housekeepingTaskId, consumptions) {
    try {
      ...
      logger.debug(`Task ${housekeepingTaskId} efficiency: ${avgEfficiency}%`);
    } catch (error) {
      logger.error('Failed to update task efficiency metrics:', error.message);
    }
  }
}
