...
async function testAdvancedReservationsDatabase() {
    ...
    try {
        ...
        totalReservations = await advancedReservations.countDocuments();
        ...
        if (totalReservations > 0) {
            ...
            vipCount = await advancedReservations.countDocuments({
                reservationType: 'vip'
            });
            console.log(`\nVIP Reservations: ${vipCount}`);

            // Count waitlist reservations
            waitlistCount = await advancedReservations.countDocuments({
            ...
            totalUpgrades = upgradeStats.length > 0 ? upgradeStats[0].totalUpgrades : 0;
        ...
        totalWaitlist = await waitingList.countDocuments();
        ...
        totalRoomBlocks = await roomBlocks.countDocuments();
        ...
        console.log(`  Total Upgrades: ${totalUpgrades}`);
        console.log(`  Waitlist Reservations: ${waitlistCount}`);
        console.log(`  VIP Reservations: ${vipCount}`);
        console.log(`  Waitlist Collection: ${totalWaitlist}`);
        console.log(`  Room Blocks: ${totalRoomBlocks}`);
        ...
        if (totalReservations === 0) {
            ...
            if (vipCount === 1 && waitlistCount === 5) {
                console.log('✅ VIP and Waitlist counts MATCH screenshot - REAL DATA confirmed');
            } else {
                console.log('⚠️  Some metrics don\'t match - Possible mixed real/mock data');
            }
        } else {
