...
class POSTaxController {
  ...
  deleteTax = catchAsync(async (req, res, next) => {
    ...
    if (isUsed) {
      // Soft delete - deactivate atomically
      const deactivated = await POSTax.findOneAndUpdate(
      ...
      logger.info('Tax deactivated', {
        taxId: deactivated._id,
        taxName: deactivated.name,
        userId: req.user._id
      });
  ...
  addTaxExemption = catchAsync(async (req, res, next) => {
    ...
    const updated = await POSTax.findOneAndUpdate(
      { _id: taxId, hotelId: req.user.hotelId },
      {
        $push: { exemptions: exemptionData },
  ...
  updateTaxExemption = catchAsync(async (req, res, next) => {
    ...
    // Build atomic $set for the subdocument fields
    const setFields = { updatedBy: req.user._id, updatedAt: new Date() };
    for (const [key, value] of Object.entries(exemptionData)) {
      setFields[`exemptions.$.${key}`] = value;
    }
    ...
    const exemption = updated.exemptions.id(exemptionId);
  ...
  deleteTaxExemption = catchAsync(async (req, res, next) => {
    ...
    const updated = await POSTax.findOneAndUpdate(
      { _id: taxId, hotelId: req.user.hotelId, 'exemptions._id': exemptionId },
      {
        $pull: { exemptions: { _id: exemptionId } },
        $set: { updatedBy: req.user._id, updatedAt: new Date() }
      },
      { new: true }
    );
