...
class GuestInventoryService {
    ...
    try {
      ...
      await session.withTransaction(async () => {
        try {
          ...
          const consumptionItemIds = consumptions.map(c => c.inventoryItemId);
          const inventoryItemsList = await InventoryItem.find({ _id: { $in: consumptionItemIds } }).session(session);
          const invItemMap = new Map(inventoryItemsList.map(it => [it._id.toString(), it]));
          ...
          for (const consumption of consumptions) {
            ...
            const inventoryItem = invItemMap.get(inventoryItemId.toString());
            if (!inventoryItem) {
  ...
  async trackGuestPreference(guestId, hotelId, inventoryItemId, preferenceData, session) {
    try {
      ...
      logger.debug('Guest preference tracked:', preferenceUpdate);

    } catch (error) {
      logger.error('Failed to track guest preference:', error.message);
    }
  }
  ...
  async createBillingEntry(consumptionRecord, booking, session) {
    try {
      ...
      logger.debug('Billing entry created:', billingEntry);

    } catch (error) {
      logger.error('Failed to create billing entry:', error.message);
    }
  }
  ...
  async isVIPGuest(guest, booking = null) {
    try {
      ...
      logger.error('Failed to check VIP status:', error.message);
      return false;
    }
  }
  ...
  async getGuestPreferences(guestId, hotelId) {
    try {
      ...
      logger.error('Failed to get guest preferences:', error.message);
