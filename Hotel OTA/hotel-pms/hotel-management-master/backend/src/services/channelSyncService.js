...
class ChannelSyncService {
  ...
  async syncHotelToChannels(hotelId, options = {}) {
    try {
      ...
      try {
        ...
        logger.debug(`📡 Found ${channels.length} active channels`);
        ...
        logger.debug(`🏨 Syncing ${roomTypes.length} room types`);
        ...
        for (const channel of channels) {
          try {
            ...
            logger.error(`Channel sync error for ${channel.name}:`, channelError);
            results.push({
              channel: channel.name,
              channelId: channel.channelId,
              success: false,
      ...
      logger.error('Hotel sync failed:', error);
      return {
        success: false,
        error: error.message
      };
  ...
  async syncChannelRatesAndAvailability(channel, roomTypes, startDate, endDate) {
    try {
      ...
      logger.debug(`📤 Syncing to ${channel.name}...`);
      ...
      for (const roomType of roomTypes) {
        try {
          ...
          if (!roomMapping) {
            logger.warn(`No mapping found for room type ${roomType.name}`);
            continue;
          }
          ...
          if (channelSyncResult.success) {
            ...
            errors++;
            logger.error(`Sync failed for room type ${roomType.name}:`, channelSyncResult.error);
          }
          ...
          errors++;
          logger.error(`Error syncing room type ${roomType.name}:`, roomError);
          syncResults.push({
            roomType: roomType.name,
            success: false,
      ...
      logger.error(`Channel sync failed for ${channel.name}:`, error);
      return {
        success: false,
        error: error.message
      };
  ...
  async getAvailabilityForSync(hotelId, roomTypes, startDate, endDate) {
    try {
      ...
      logger.error('Failed to get availability for sync:', error);
      return [];
    }
  }
  ...
  async handleIncomingReservation(channelCategory, reservationData) {
    try {
      ...
      logger.error('Failed to handle incoming reservation:', error);
      return {
        success: false,
        error: error.message
      };
  ...
  async updateChannelInventoryAfterBooking(booking, action) {
    try {
      ...
      logger.debug(`📋 Updating channel inventory after booking ${action}`);
      ...
      logger.error('Failed to update channel inventory:', error);
    }
  }
