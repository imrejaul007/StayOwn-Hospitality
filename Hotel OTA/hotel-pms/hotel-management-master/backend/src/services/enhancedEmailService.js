...
class EnhancedEmailService {
  ...
  async sendCampaign(campaignId, testEmail = null) {
    try {
      ...
      logger.debug(`📧 Starting email campaign: ${campaign.name}`);
      ...
      logger.debug(`👥 Found ${recipients.length} recipients`);
      ...
      logger.debug(`✅ Campaign completed: ${results.sent} sent, ${results.failed} failed`);
      return results;
