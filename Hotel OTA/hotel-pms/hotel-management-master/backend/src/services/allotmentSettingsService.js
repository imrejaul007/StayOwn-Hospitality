...
class AllotmentSettingsService {
  ...
  async getHotelSettings(hotelId, userId) {
    try {
      logger.debug('🔍 [AllotmentSettingsService] Fetching hotel settings for:', hotelId);

      let settings = await HotelAllotmentSettings.findByHotelId(hotelId);

      if (!settings) {
        logger.debug('🆕 [AllotmentSettingsService] No settings found, creating default settings');
      ...
      logger.debug('✅ [AllotmentSettingsService] Settings fetched successfully');
      return settings;
    } catch (error) {
      logger.error('❌ [AllotmentSettingsService] Error fetching hotel settings:', error);
  ...
  async updateGlobalSettings(hotelId, globalSettings, userId) {
    try {
      logger.debug('🔧 [AllotmentSettingsService] Updating global settings for hotel:', hotelId);
      ...
      logger.debug('✅ [AllotmentSettingsService] Global settings updated successfully');
      return settings;
    } catch (error) {
      logger.error('❌ [AllotmentSettingsService] Error updating global settings:', error);
  ...
  async updateIntegrationSettings(hotelId, integrationSettings, userId) {
    try {
      logger.debug('🔧 [AllotmentSettingsService] Updating integration settings for hotel:', hotelId);
      ...
      logger.debug('✅ [AllotmentSettingsService] Integration settings updated successfully');
      return settings;
    } catch (error) {
      logger.error('❌ [AllotmentSettingsService] Error updating integration settings:', error);
  ...
  async updateAnalyticsSettings(hotelId, analyticsSettings, userId) {
    try {
      logger.debug('🔧 [AllotmentSettingsService] Updating analytics settings for hotel:', hotelId);
      ...
      logger.debug('✅ [AllotmentSettingsService] Analytics settings updated successfully');
      return settings;
    } catch (error) {
      logger.error('❌ [AllotmentSettingsService] Error updating analytics settings:', error);
  ...
  async addAllocationRuleTemplate(hotelId, templateData, userId) {
    try {
      logger.debug('🎯 [AllotmentSettingsService] Adding allocation rule template for hotel:', hotelId);
      ...
      logger.debug('✅ [AllotmentSettingsService] Allocation rule template added successfully');
      return settings;
    } catch (error) {
      logger.error('❌ [AllotmentSettingsService] Error adding allocation rule template:', error);
  ...
  async deleteAllocationRuleTemplate(hotelId, templateId, userId) {
    try {
      logger.debug('🗑️ [AllotmentSettingsService] Deleting allocation rule template:', templateId);
      ...
      logger.debug('✅ [AllotmentSettingsService] Allocation rule template deleted successfully');
      return settings;
    } catch (error) {
      logger.error('❌ [AllotmentSettingsService] Error deleting allocation rule template:', error);
  ...
  async testIntegration(hotelId, integrationType, config) {
    try {
      logger.debug('🧪 [AllotmentSettingsService] Testing integration:', integrationType);
      ...
      logger.debug('✅ [AllotmentSettingsService] Integration test completed:', results);
      return results;
    } catch (error) {
      logger.error('❌ [AllotmentSettingsService] Error testing integration:', error);
      return {
        success: false,
  ...
  async testChannelManagerConnection(config) {
    try {
      ...
      logger.error('❌ Channel manager test error:', error);
      return false;
    }
  }
  ...
  async testPMSConnection(config) {
    try {
      ...
      logger.error('❌ PMS test error:', error);
      return false;
    }
  }
  ...
  async resetToDefaults(hotelId, userId) {
    try {
      logger.debug('🔄 [AllotmentSettingsService] Resetting settings to defaults for hotel:', hotelId);
      ...
      logger.debug('✅ [AllotmentSettingsService] Settings reset to defaults successfully');
      return settings;
    } catch (error) {
      logger.error('❌ [AllotmentSettingsService] Error resetting settings:', error);
  ...
  async exportSettings(hotelId) {
    try {
      logger.debug('📤 [AllotmentSettingsService] Exporting settings for hotel:', hotelId);
      ...
      logger.debug('✅ [AllotmentSettingsService] Settings exported successfully');
      return exportData;
    } catch (error) {
      logger.error('❌ [AllotmentSettingsService] Error exporting settings:', error);
  ...
  async importSettings(hotelId, importData, userId) {
    try {
      logger.debug('📥 [AllotmentSettingsService] Importing settings for hotel:', hotelId);
      ...
      logger.debug('✅ [AllotmentSettingsService] Settings imported successfully');
      return settings;
    } catch (error) {
      logger.error('❌ [AllotmentSettingsService] Error importing settings:', error);
  ...
  async getSettingsSummary(hotelId) {
    try {
      ...
      logger.error('❌ [AllotmentSettingsService] Error getting settings summary:', error);
