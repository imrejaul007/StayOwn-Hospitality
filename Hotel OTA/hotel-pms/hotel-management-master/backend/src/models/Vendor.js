...
import { getPIIEncryption } from '../utils/piiEncryption.js';
...
vendorSchema.pre('save', function(next) {
  const pii = getPIIEncryption();
  // Encrypt nested tax info PII fields
  if (this.taxInfo) {
    if (this.isModified('taxInfo.gstNumber') && this.taxInfo.gstNumber && !pii.isEncrypted(this.taxInfo.gstNumber)) {
      this.taxInfo.gstNumber = pii.encrypt(this.taxInfo.gstNumber);
    }
    if (this.isModified('taxInfo.panNumber') && this.taxInfo.panNumber && !pii.isEncrypted(this.taxInfo.panNumber)) {
      this.taxInfo.panNumber = pii.encrypt(this.taxInfo.panNumber);
    }
  }
  // Encrypt bank account number
  if (this.bankDetails) {
    if (this.isModified('bankDetails.accountNumber') && this.bankDetails.accountNumber && !pii.isEncrypted(this.bankDetails.accountNumber)) {
      this.bankDetails.accountNumber = pii.encrypt(this.bankDetails.accountNumber);
    }
  }
  next();
});

// Method to get masked PII values
vendorSchema.methods.getMaskedPII = function(fieldName) {
  const pii = getPIIEncryption();
  // Support dot notation for nested fields
  const value = fieldName.split('.').reduce((obj, key) => obj && obj[key], this);
  return value ? pii.mask(value) : null;
};
