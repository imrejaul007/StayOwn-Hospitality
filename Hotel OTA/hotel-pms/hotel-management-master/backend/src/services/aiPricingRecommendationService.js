...
class AIPricingRecommendationService {
  ...
  async generatePricingRecommendations(filters = {}) {
    try {
      ...
      logger.error('Error in generatePricingRecommendations:', error);
      return {
        success: false,
  ...
  async getHistoricalPricingData(hotelId, roomTypeId) {
    try {
      ...
      logger.error('Error getting historical pricing data:', error);
      return [];
    }
  }
  ...
  async calculateDemandTrends(hotelId, roomTypeId) {
    try {
      ...
      logger.error('Error calculating demand trends:', error);
  ...
  async exportPricingReport(filters = {}) {
    try {
      ...
      logger.error('Error exporting pricing report:', error);
      return {
        success: false,
