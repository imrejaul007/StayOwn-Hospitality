...
class RoomFeatureService {
  ...
  async getFeaturesByCategories(hotelId, options = {}) {
    try {
      ...
      const features = await RoomFeature.find(query)
        ...
        .sort({ category: 1, 'displaySettings.displayOrder': 1, featureName: 1 }).lean().limit(1000);
  ...
  async searchFeatures(hotelId, searchCriteria = {}) {
    try {
      ...
      const features = await RoomFeature.find(query)
        ...
        .sort(sortOptions).lean().limit(1000);
  ...
  async getRoomFeatures(roomId) {
    try {
      const features = await RoomFeature.find({
        ...
        status: 'active'
      }).sort({ category: 1, 'displaySettings.displayOrder': 1 }).lean().limit(1000);
  ...
  async bulkAssignFeaturesToRooms(featureIds, roomIds) {
    ...
    // Batch: fetch all features in a single query
    const features = await RoomFeature.find({ _id: { $in: featureIds } });
    const featureMap = new Map(features.map(f => [f._id.toString(), f]));

    for (const featureId of featureIds) {
      const feature = featureMap.get(featureId.toString());
      if (!feature) {
        results.failed.push({
          featureId,
  ...
  async checkFeatureAvailability(featureId, startDate, endDate) {
    try {
      const feature = await RoomFeature.findById(featureId).lean();
  ...
  async updateAllFeatureStatistics(hotelId) {
    const features = await RoomFeature.find({ hotelId }).lean().limit(1000);
    const results = [];
    ...
    try {
      ...
      for (const sourceFeature of sourceFeatures) {
        ...
        try {
          ...
          logger.error(`Error cloning feature ${sourceFeature.featureName}:`, error);
        }
      }
