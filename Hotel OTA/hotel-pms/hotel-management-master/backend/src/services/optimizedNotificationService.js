...
class OptimizedNotificationService {
    ...
    try {
      ...
      if (!skipRateLimit) {
        ...
        if (!rateLimitCheck.allowed) {
          ...
          logger.warn('Notification blocked by rate limit:', rateLimitCheck.reason);
          return {
            success: false,
            reason: 'rate_limited',
  ...
  async processBatch() {
    ...
    try {
      ...
      logger.debug(`Processed batch of ${batch.length} notifications`);
  ...
  async preloadTemplates(hotelId) {
    try {
      ...
      logger.debug(`Preloaded ${commonTypes.length} templates for hotel ${hotelId}`);
    } catch (error) {
  ...
  async warmupUserCache(userIds) {
    try {
      ...
      logger.debug(`Warmed up cache for ${userIds.length} users`);
    } catch (error) {
  ...
  async cleanup() {
    try {
      ...
      logger.debug(`Cleanup completed: deleted ${deleteResult.deletedCount}, archived ${archiveResult.modifiedCount}`);
