...
class NotificationRoutingService {
  ...
  async filterRecipientsByPreferences(users, notificationType, priority) {
    ...
    for (const user of users) {
      try {
        ...
        logger.error(`Error checking preferences for user ${user._id}:`, error);
  ...
  async trackRoutingPerformance(notificationId, routingPlan, actualResults) {
    try {
      ...
      logger.debug('Routing performance:', performance);
      return performance;
    } catch (error) {
      logger.error('Error tracking routing performance:', error);
    }
  }
