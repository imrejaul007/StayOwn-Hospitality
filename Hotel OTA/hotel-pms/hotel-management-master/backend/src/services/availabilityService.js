...
class AvailabilityService {
  ...
  async unblockRooms(roomIds, startDate, endDate) {
    try {
      ...
      logger.error('Error unblocking rooms:', error);
      throw error;
    }
  }
  ...
  async getRoomAvailabilityStatus(roomId, startDate, endDate) {
    try {
      ...
      logger.error('Error getting room availability status:', error);
      throw error;
    }
  }
  ...
  async handleOverbooking(date, roomType = null, hotelId = null) {
    try {
      ...
      logger.error('Error handling overbooking:', error);
      throw error;
    }
  }
  ...
  async releaseRooms({ hotelId, roomTypeId, checkIn, checkOut, roomsCount, bookingId, userId }) {
    try {
      ...
      logger.error('Room release failed:', error);
      return {
        success: false,
  ...
  async getRoomTypeOptions(hotelId) {
    try {
      ...
      logger.error('Failed to get room type options:', error);
      return [];
    }
  }
