...
class BookingComService {
  ...
  async initializeChannel({ hotelId, credentials, settings = {} }) {
    try {
      ...
      logger.error('Failed to initialize Booking.com channel:', error);
      return {
        success: false,
        error: error.message
      };
  ...
  async testConnection(channel) {
    try {
      ...
      logger.error('Booking.com connection test failed:', error);
      return {
        success: false,
  ...
  async createRoomMappings(channelId, mappings) {
    try {
      ...
      logger.error('Failed to create room mappings:', error);
      return {
        success: false,
        error: error.message
      };
      ...
      for (const roomMapping of channel.roomMappings) {
        try {
          ...
          if (syncResult.success) {
            ...
            errors++;
            logger.error(`Sync failed for room type ${roomMapping.channelRoomTypeName}:`, syncResult.error);
          }
          ...
          errors++;
          logger.error(`Error syncing room type ${roomMapping.channelRoomTypeName}:`, roomError);
  ...
  async sendToBookingCom(channel, syncData) {
    try {
      ...
      */
      ...
      logger.error('Failed to send data to Booking.com:', error);
      return {
        success: false,
        error: error.message
      };
  ...
  async handleIncomingReservation(reservationData) {
    try {
      ...
      logger.error('Failed to process Booking.com reservation:', error);
      return {
        success: false,
        error: error.message
      };
