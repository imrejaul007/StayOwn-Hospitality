...
<!-- End Hotel Booking Widget -->`;
    ...
    try {
      ...
      if (bookingData.promoCode) {
        ...
        if (promoValidation.valid) {
          ...
          // Atomically increment promo code usage counter
      ...
      logger.error('Error processing widget booking', { error: error.message });
      throw error;
    }
  }
  ...
  async updateGuestCRM(guestData, booking) {
    try {
      ...
      logger.error('Error updating guest CRM', { error: error.message });
    }
  }
  ...
  async sendEmailCampaign(campaignId, testEmail = null) {
    try {
      ...
      logger.error('Error sending email campaign', { campaignId, error: error.message });
      throw error;
    }
  }
  ...
  async trackEmailEvent(campaignId, email, event, data = {}) {
    try {
      ...
      logger.error('Error tracking email event', { campaignId, event, error: error.message });
    }
  }
  ...
  async generateLoyaltyPoints(guestId, action, bookingAmount = 0) {
    try {
      ...
      logger.error('Error generating loyalty points', { guestId, action, error: error.message });
      return 0;
    }
  }
  ...
  async processReview(reviewData) {
    try {
      ...
      logger.error('Error processing review', { error: error.message });
      throw error;
    }
  }
  ...
  async sendBookingConfirmationEmail(booking) {
    try {
      ...
      logger.error('Error sending booking confirmation', { bookingId: booking?.bookingId, error: error.message });
    }
  }
  ...
  async generateAutomatedCampaigns() {
    try {
      ...
      logger.error('Error generating automated campaigns', { error: error.message });
    }
  }
