 ...
 * M-4: Updated to async/await for Redis-based session management
 */
export const validateSession = async (req, res, next) => {
  ...
  try {
    ...
    const hasSession = await bypassSecurityService.checkSessionTimeout(sessionId);
    if (!hasSession) {
      // Set new session timeout for this bypass operation
      await bypassSecurityService.setSessionTimeout(sessionId);
    }
    next();
  } catch (error) {
    logger.error('Session timeout check failed', { sessionId, error: error.message });
    // Fail open - allow operation if session check fails
    next();
  }
};
