...
export const checkoutSession = catchAsync(async (req, res) => {
  ...
  // Atomically claim the session for checkout by transitioning status
  // from 'draft' to the payment-determined status in a single operation.
  // This prevents two concurrent checkout requests from both succeeding.
  const targetStatus = paymentMethod === 'room_charge' ? 'room_charged' : 'paid';
  ...
  if (splitPayments && splitPayments.length > 0) {
    updateFields.splitPayments = splitPayments;
  }
...
export const voidSession = catchAsync(async (req, res) => {
  ...
  // Atomically transition status to 'void', preventing double-void
  // and race conditions where two requests both read a non-void status.
  const updateFields = { status: 'void', voidedAt: new Date() };
  if (reason) {
