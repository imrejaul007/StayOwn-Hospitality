...
// Connect channel
router.post('/connect', requireRole('hotel_admin', 'super_admin'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { hotelId, otaType, credentials, settings } = req.body;

    // Check if already connected
    const existing = await Channel.findOne({ hotelId, otaType });
    if (existing) {
...
// Get channel status
router.get('/status/:hotelId', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const channels = await Channel.find({ hotelId: req.params.hotelId });
...
router.get('/bookings/:hotelId', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { startDate, endDate, channel } = req.query;

    const query: any = { hotelId: req.params.hotelId };
    if (startDate && endDate) {
      query.checkIn = { $gte: new Date(startDate as string), $lte: new Date(endDate as string) };
    }
    if (channel) {
      query.channelType = channel;
    }

    const bookings = await ChannelBooking.find(query).sort({ checkIn: -1 });
...
router.post('/booking/confirm', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const {
      hotelId,
      channelBookingId,
