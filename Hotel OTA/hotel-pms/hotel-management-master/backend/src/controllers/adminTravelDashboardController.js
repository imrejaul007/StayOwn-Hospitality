...
export const createComprehensiveExport = catchAsync(async (req, res) => {
  ...
  // Add analytics data if requested — run in parallel to avoid sequential overhead
  if (includeAnalytics) {
    try {
      const [analyticsData, performanceData] = await Promise.all([
        analyticsService.analyzeBookingTrends(filters),
        analyticsService.calculatePerformanceMetrics(filters)
      ]);
