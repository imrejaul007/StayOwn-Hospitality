...
class SmartSegmentationService {
  ...
  async performAdvancedSegmentation(hotelId, options = {}) {
    try {
      ...
      logger.error('Error in advanced segmentation:', error);
      throw error;
    }
  }
  ...
  async calculateProfileAnalytics(profiles) {
    ...
    for (const profile of profiles) {
      try {
        ...
        logger.error(`Error calculating analytics for profile ${profile._id}:`, error);
  ...
  async getSegmentInsights(hotelId, segmentId) {
    try {
      ...
      logger.error('Error getting segment insights:', error);
      throw error;
    }
  }
