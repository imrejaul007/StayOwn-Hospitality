...
class GuestServicePOSIntegrationService {
  ...
  async createPOSOrderFromServiceRequest(serviceRequest, options = {}) {
    try {
      ...
      if (!this.isFoodServiceRequest(serviceRequest)) {
        logger.debug('Service request is not food-related, skipping POS order creation');
        return null;
      }
      ...
      logger.debug(`POS order ${posOrder.orderNumber} created from service request ${serviceRequest._id}`);
      ...
      logger.error('Error creating POS order from service request:', error);
      throw error;
    }
  }
  ...
  async findAppropriateOutlet(serviceRequest, hotelId) {
    try {
      ...
      logger.error('Error finding appropriate outlet:', error);
      return null;
    }
  }
  ...
  async calculateOrderTotals(orderData, outlet) {
    ...
    try {
      ...
      logger.warn('Tax calculation service failed, using outlet defaults:', taxError.message);
  ...
  async notifyKitchenStaff(posOrder, outlet) {
    try {
      ...
      logger.error('Error sending kitchen staff notifications:', error);
  ...
  async linkServiceRequestToPOSOrder(serviceRequest, posOrder) {
    try {
      ...
      logger.debug(`Service request ${serviceRequest._id} linked to POS order ${posOrder.orderNumber}`);
    } catch (error) {
      logger.error('Error linking service request to POS order:', error);
    }
  }
}
