...
class InventoryController {
  ...
  async bulkUpdateInventory(req, res) {
    try {
      ...
      // Batch: fetch all existing inventory for the update dates at once
      const updateDates = updates.map(u => new Date(u.date));
      const existingInventory = await RoomAvailability.find({
        hotelId,
        roomTypeId,
        date: { $in: updateDates }
      }).lean().limit(1000);
      ...
      for (const update of updates) {
        try {
          ...
          let inventory = inventoryByDate.get(dateKey);
      ...
      if (auditLogs.length > 0) {
        await Promise.all(auditLogs.map(auditData => AuditLog.logChange(auditData)));
      }
  ...
  async createInventoryRange(req, res) {
    try {
      ...
      // Batch query: fetch all existing inventory records for the date range at once
      const existingRecords = await RoomAvailability.find({
        hotelId,
        roomTypeId,
      ...
      existingRecords.forEach(rec => {
        existingByDate.set(rec.date.toISOString().split('T')[0], rec);
      });

      // If overwrite mode, batch delete all existing records in the range
      if (createMode === 'overwrite' && existingRecords.length > 0) {
        const idsToDelete = existingRecords.map(rec => rec._id);
        await RoomAvailability.deleteMany({ _id: { $in: idsToDelete } });
      }
