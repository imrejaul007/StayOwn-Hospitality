...
class RevenueManagementService {
  ...
  async calculateRevenueKPIs(hotelId, startDate, endDate, options = {}) {
    try {
      ...
      logger.error('Error calculating revenue KPIs:', error);
      return {
        success: false,
        error: error.message
      };
  ...
  async runYieldManagementAnalysis(hotelId, options = {}) {
    try {
      ...
      logger.debug(`📈 Running yield management analysis for hotel ${hotelId}...`);
      ...
      logger.error('Error in yield management analysis:', error);
      return {
        success: false,
        error: error.message
      };
