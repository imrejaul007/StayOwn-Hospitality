...
// Always add console transport so Render/cloud log panels receive output
logger.add(new winston.transports.Console({
  format: winston.format.combine(
    // Skip colorize in production — it adds ANSI escape codes that pollute cloud logs
    ...(process.env.NODE_ENV !== 'production' ? [winston.format.colorize()] : []),
    winston.format.timestamp({ format: 'YYYY-MM-DD HH:mm:ss' }),
    winston.format.printf(({ timestamp, level, message, stack, ...meta }) => {
      // Strip internal Winston/default meta that adds noise
      const { service, environment, version, ...rest } = meta;
...
// Also stream exceptions/rejections to console in production
