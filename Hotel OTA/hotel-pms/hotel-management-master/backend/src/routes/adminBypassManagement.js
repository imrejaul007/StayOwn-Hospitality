...
router.post('/enhanced-checkout',
    ...
    catchAsync(async (req, res) => {
        ...
        try {
            ...
            // H-6: Wrap all database operations in a MongoDB transaction for atomicity
            // This ensures all-or-nothing semantics: if any operation fails, none are committed
            const session = await mongoose.startSession();
            let checkoutInventory;
            ...
            try {
                ...
                checkoutInventory = await CheckoutInventory.create([{
                bookingId: booking._id,
                roomId: booking.rooms[0].roomId._id,
                checkedBy: adminId,
            ...
            auditRecord.checkoutInventoryId = checkoutInventory[0]._id;
            ...
            await checkoutInventory[0].populate([{
                    path: 'bookingId',
            ...
            res.status(200).json({
                ...
                data: {
                    ...
                    checkoutInventory: {
                        id: checkoutInventory[0]._id,
                        totalAmount: checkoutInventory[0].totalAmount,
                        paymentMethod: checkoutInventory[0].paymentMethod,
                        notes: checkoutInventory[0].notes,
                        isAdminBypass: checkoutInventory[0].isAdminBypass
                    },
                    securitySummary: {
                        riskScore: securityResult.riskScore,
                        riskLevel: getRiskLevel(securityResult.riskScore),
            ...
            // H-6: Commit transaction - all operations succeeded
            await session.commitTransaction();
            session.endSession();
        } catch (error) {
            // H-6: Abort transaction on any failure - rollback all changes
            ...
            session.endSession();
                ...
                if (auditRecord) {
                    try {
                        ...
                        logger.error('Failed to update audit record after transaction abort', {
                            bypassId: auditRecord?.bypassId,
            ...
            // Outer error handler for unexpected failures
            console.error('Enhanced checkout failed:', outerError);
            throw outerError;
        }
    })
);
...
async function executeApprovedBypass(workflow) {
    try {
        ...
        logger.info('Approved bypass executed successfully', {
            workflowId: workflow.workflowId,
            bypassId: auditRecord.bypassId,
            bookingNumber: booking.bookingNumber
        });
        ...
        logger.error('Failed to execute approved bypass', { error: error.message });
