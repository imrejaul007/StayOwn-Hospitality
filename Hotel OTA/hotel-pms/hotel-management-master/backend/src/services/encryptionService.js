...
class EncryptionService {
   ...
   */
  initializeKeys() {
    // Fall back to ENCRYPTION_KEY if specific keys aren't set
    const fallbackKey = process.env.ENCRYPTION_KEY || null;
    const requiredKeys = {
      PII_ENCRYPTION_KEY: process.env.PII_ENCRYPTION_KEY || fallbackKey,
      CREDENTIAL_ENCRYPTION_KEY: process.env.CREDENTIAL_ENCRYPTION_KEY || fallbackKey,
      FINANCIAL_ENCRYPTION_KEY: process.env.FINANCIAL_ENCRYPTION_KEY || fallbackKey
    };
    ...
    if (missingKeys.length > 0) {
      logger.warn(`Missing encryption keys: ${missingKeys.join(', ')}. Using fallback or degraded mode.`);
      // In development, use a default key instead of crashing
      const devKey = 'dev-default-encryption-key-change-in-production-' + Date.now();
      missingKeys.forEach(key => {
        requiredKeys[key] = devKey;
      });
    }

    this.piiKey = this.deriveKey(requiredKeys.PII_ENCRYPTION_KEY, 'PII_ENCRYPTION');
    this.credentialKey = this.deriveKey(requiredKeys.CREDENTIAL_ENCRYPTION_KEY, 'CREDENTIAL_ENCRYPTION');
    this.financialKey = this.deriveKey(requiredKeys.FINANCIAL_ENCRYPTION_KEY, 'FINANCIAL_ENCRYPTION');
