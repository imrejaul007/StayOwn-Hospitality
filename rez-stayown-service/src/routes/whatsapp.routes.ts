 ...
 */
router.post('/send', validateApiKey, async (req: Request, res: Response) => {
  try {
    ...
    // Format phone number
    const formattedPhone = whatsappService.formatPhoneNumber(phone, countryCode || '91');
    ...
    // Send message
    const result: WhatsAppMessageResult = await whatsappService.sendMessage(
      formattedPhone,
      template,
      params
    );
 ...
 *   - countryCode?: string
 */
router.post('/send/booking-confirmation', validateApiKey, async (req: Request, res: Response) => {
  try {
    ...
    const result = await whatsappService.sendBookingConfirmation(formattedPhone, {
      hotelName,
      guestName,
      checkIn,
      roomNumber
 ...
 *   - countryCode?: string
 */
router.post('/send/checkin-reminder', validateApiKey, async (req: Request, res: Response) => {
  try {
    ...
    if (!qrUrl) missingFields.push('qrUrl');
    ...
    const result = await whatsappService.sendCheckinReminder(formattedPhone, {
      guestName,
      hotelName,
      checkInTime,
      qrUrl
 ...
 *   - countryCode?: string
 */
router.post('/send/service-ready', validateApiKey, async (req: Request, res: Response) => {
  try {
    ...
    const result = await whatsappService.sendServiceReady(formattedPhone, {
      guestName,
      orderId
    });
 ...
 *     }>
 */
router.post('/send/bulk', validateApiKey, async (req: Request, res: Response) => {
  try {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      const formattedPhone = whatsappService.formatPhoneNumber(msg.phone, msg.countryCode || '91');
 ...
 */
router.get('/templates', validateApiKey, (_req: Request, res: Response) => {
  return res.json({
    success: true,
    templates: [
      {
 ...
 */
router.post('/test', validateApiKey, async (req: Request, res: Response) => {
  try {
...
async function handleIncomingMessage(message: any): Promise<void> {
  ...
  if (message.text?.body?.toUpperCase() === 'HELP') {
    await whatsappService.sendTextMessage(
      message.from,
      'Welcome to StayOwn! Available commands:\n' +
