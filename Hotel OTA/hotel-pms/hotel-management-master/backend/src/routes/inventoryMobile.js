...
router.get('/quick-stock-check', [
  ...
  try {
    ...
    logger.error('Error in quick stock check', { error: error.message });
    res.status(500).json({
      success: false,
  ...
  try {
    ...
    switch (movementType) {
      ...
      case 'OUT':
        ...
        if (previousStock < quantity) {
          logger.warn('Attempting to remove more units than available', { itemName: item.name, requested: quantity, available: previousStock });
        }
        break;
      case 'ADJUSTMENT':
    ...
    if (photo) {
      try {
        ...
        logger.error('Error processing photo', { error: photoError.message });
    ...
    await session.abortTransaction();
    logger.error('Error in stock update', { error: error.message });
    res.status(500).json({
      success: false,
...
router.get('/barcode-lookup/:barcode', [
  ...
  try {
    ...
    logger.error('Error in barcode lookup', { error: error.message });
    res.status(500).json({
      success: false,
...
router.post('/offline-sync', [
  ...
  try {
    ...
    logger.error('Error in offline sync', { error: error.message });
    res.status(500).json({
      success: false,
...
router.post('/push-notifications', [
  ...
  try {
    ...
    logger.error('Error registering for push notifications', { error: error.message });
    res.status(500).json({
      success: false,
