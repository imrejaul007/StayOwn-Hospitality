...
inventoryHistorySchema.statics.getTrendData = async function(hotelId, itemId, days = 30) {
  try {
    ...
    .sort({ snapshotDate: 1 }).lean().limit(1000);
  } catch (error) {
    throw new Error(`${error.message}`);
  }
};
...
inventoryHistorySchema.statics.generatePredictiveInsights = async function(hotelId, daysAhead = 30) {
  try {
    ...
    const currentItems = await InventoryItem.find({ hotelId, isActive: true }).lean().limit(1000);
...
// Data retention TTL: auto-delete inventory history records after 2 years
inventoryHistorySchema.index({ createdAt: 1 }, { expireAfterSeconds: 730 * 24 * 60 * 60 });
