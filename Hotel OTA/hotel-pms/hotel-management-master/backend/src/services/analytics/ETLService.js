...
class ETLService {
  ...
  async updateAggregates() {
    try {
      ...
      // Stream all hotels via cursor — avoids hard limit of 1000 that would silently
      // skip analytics aggregation for any hotels beyond the first thousand.
      const HOTEL_BATCH = 100;
      let hotelSkip = 0;
      let hotels = [];
      do {
        hotels = await Hotel.find({}).select('_id').lean().skip(hotelSkip).limit(HOTEL_BATCH);
        for (const hotel of hotels) {
          await this.updateMonthlyRevenueAggregate(hotel._id, year, month);
        }
        hotelSkip += HOTEL_BATCH;
      } while (hotels.length === HOTEL_BATCH);
