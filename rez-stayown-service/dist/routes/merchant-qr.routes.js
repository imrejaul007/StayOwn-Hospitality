 ...
 */
router.post('/scan', auth_1.authenticateService, rateLimiter_1.rateLimiters.qrValidate, async (req, res) => {
    try {
        ...
        // Get full QR details
        const qrRecord = await (0, room_qr_1.getRoomQRByToken)(token);
        if (!qrRecord) {
            res.status(404).json({
                success: false,
 ...
 */
router.post('/checkin', auth_1.authenticateService, rateLimiter_1.rateLimiters.general, async (req, res) => {
    try {
        ...
        try {
            await axios_1.default.post(`${HOTEL_PMS_URL}/v1/pms/inventory/checkin`, {
                bookingId: validation.bookingId,
        ...
        res.json({
            ...
            data: {
                bookingId: validation.bookingId,
                roomNumber: validation.roomNumber,
                guestName: validation.bookingId ? (await (0, room_qr_1.getRoomQRByBookingId)(validation.bookingId))?.guestName : undefined,
            },
        });
    }
    catch (error) {
 ...
 */
router.get('/booking/:token', auth_1.authenticateService, rateLimiter_1.rateLimiters.qrValidate, async (req, res) => {
    try {
        ...
        try {
            const response = await axios_1.default.get(`${HOTEL_PMS_URL}/v1/bookings/${validation.bookingId}`, {
                timeout: 5000,
                headers: {
                    'x-service-key': process.env.INTERNAL_SERVICE_TOKEN || '',
                },
 ...
 */
router.post('/verify-access', auth_1.authenticateService, rateLimiter_1.rateLimiters.qrValidate, async (req, res) => {
    try {
...
//# sourceMappingURL=merchant-qr.routes.js.map