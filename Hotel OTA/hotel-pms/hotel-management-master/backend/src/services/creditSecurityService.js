...
class CreditSecurityService {
  ...
  async logCreditOperation(operationData) {
    try {
      ...
      logger.debug('CREDIT_AUDIT_LOG:', JSON.stringify(logEntry));
      ...
      if (operationData.severity === 'critical') {
        ...
        logger.debug('CRITICAL_OPERATION_LOGGED:', {
          operation: operationData.operation,
          userId: operationData.userId,
  ...
  async verifyTransactionIntegrity(transactionId) {
    try {
      ...
      logger.error('Transaction integrity check failed:', error);
      return {
        isValid: false,
  ...
  async validateCreditLimitChange(companyId, currentLimit, newLimit, userId) {
    try {
      ...
      logger.error('Credit limit validation failed:', error);
      return {
        isValid: false,
        error: error.message,
        riskLevel: 'unknown'
