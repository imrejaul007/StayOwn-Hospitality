...
class BypassSecurityService {
  constructor() {
    ...
    // M-4: Removed in-memory sessionTimeouts Map - now uses Redis with TTL
    this.SESSION_TIMEOUT_TTL_SECONDS = 300; // 5 minutes
    this.encryptionKey = process.env.BYPASS_ENCRYPTION_KEY || process.env.ENCRYPTION_KEY || 'dev-bypass-key-change-in-production';
  }
  ...
  validateBypassSecurity() {
    return async (req, res, next) => {
      try {
        ...
        logger.error('Bypass security validation error:', error);
        next(error);
      }
    };
  }
  ...
  /**
   * Session timeout management using Redis with TTL
   * M-4: Uses Redis instead of in-memory Map for multi-instance support
   */
  async setSessionTimeout(sessionId, timeoutMs = 5 * 60 * 1000) {
    const redis = getRedisClient();
    const ttlSeconds = Math.ceil(timeoutMs / 1000);
    const key = `bypass:session:${sessionId}`;

    if (redis && redis.isReady) {
      try {
        await redis.setEx(key, ttlSeconds, '1');
      } catch (error) {
        logger.error('Failed to set session timeout in Redis', { sessionId, error: error.message });
        // Fallback: don't set timeout if Redis fails
      }
    }
    // Note: No need to track in memory - Redis handles TTL automatically
  }
  ...
  async clearSessionTimeout(sessionId) {
    ...
    if (redis && redis.isReady) {
      try {
        ...
        logger.error('Failed to clear session timeout in Redis', { sessionId, error: error.message });
      }
    }
  }
  ...
  async checkSessionTimeout(sessionId) {
    ...
    if (redis && redis.isReady) {
      try {
        ...
        logger.error('Failed to check session timeout in Redis', { sessionId, error: error.message });
        // Fallback to true (allow) if Redis fails
        return true;
      }
    }
    // If Redis unavailable, allow bypass (fail open for availability)
    return true;
  }
