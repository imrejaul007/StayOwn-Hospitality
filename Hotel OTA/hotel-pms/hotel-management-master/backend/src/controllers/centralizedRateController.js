...
export const updateRate = catchAsync(async (req, res) => {
  ...
  // Fetch the rate before update to get previous values for webhook
  const existingRate = await CentralizedRate.findById(rateId).lean();
  const previousRate = existingRate?.rate;
  const previousAvailability = existingRate?.availability;
  ...
  // PMS→OTA: Emit pricing_changed webhook when rate changes (fire-and-forget)
  if ((updateData.rate !== undefined || updateData.availability !== undefined) && rate.hotelId) {
    try {
      const Hotel = (await import('../models/Hotel.js')).default;
      const RoomType = (await import('../models/RoomType.js')).default;

      const hotel = await Hotel.findById(rate.hotelId).lean();
      const roomType = await RoomType.findById(rate.roomTypeId).lean();
      ...
      if (hotel && roomType) {
        await pmsOtaIntegration.emitPricingChanged(
          ...
          updateData.rate || rate.rate,
          previousAvailability || rate.availability,
          updateData.availability || rate.availability,
          'Rate updated via PMS'
        );
      }
    } catch (webhookErr) {
      logger.warn('[PMS→OTA] Pricing changed webhook emission failed (non-blocking):', webhookErr.message);
    }
  }
});
