...
class EmailTemplateEngine {
  ...
  async initialize() {
    try {
      ...
      logger.debug('📧 Email Template Engine initialized successfully');
    } catch (error) {
      logger.error('❌ Failed to initialize Email Template Engine:', error);
    }
  }
  ...
  async createTemplate(name, htmlContent, description = '') {
    try {
      ...
      logger.debug(`📧 Created email template: ${name}`);
      return true;
    } catch (error) {
  ...
  async updateTemplate(name, htmlContent) {
    try {
      ...
      logger.debug(`📧 Updated email template: ${name}`);
      return true;
    } catch (error) {
  ...
  async deleteTemplate(name) {
    try {
      ...
      logger.debug(`📧 Deleted email template: ${name}`);
      return true;
    } catch (error) {
  ...
  async renderWithPersonalization(templateName, user, campaignData = {}, additionalData = {}) {
    try {
      ...
      logger.error('Error rendering personalized template:', error);
